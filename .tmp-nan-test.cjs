...
function run(label, tree, metric = "size") {
  const root = hierarchy(structuredClone(tree)).sum((d) => !d.children || d.children.length === 0 ? d.attributes?.[metric] ?? 0 : 0);
  treemap().size([400, 400]).numberOfPasses(2).margin(0.01).floorLabels(3).labelLength(0.03)(root);
  let nan = 0, zero = 0, total = 0;
  root.each((n) => {
    total++;
    const w = n.x1 - n.x0, h = n.y1 - n.y0;
    if (!Number.isFinite(w) || !Number.isFinite(h)) nan++;
    else if (w <= 0 || h <= 0) zero++;
  });
  console.log(label.padEnd(34), "| knoten", total, "| NaN", nan, "| nullflaeche", zero);
}
run("flare (bundled, alle > 0)", JSON.parse((0, import_fs.readFileSync)("demo/src/lib/data/flare.json", "utf8")));
run("sample (bundled, alle > 0)", JSON.parse((0, import_fs.readFileSync)("demo/src/lib/data/sample.json", "utf8")));
...
var onlyPositive = { name: "root", children: [
