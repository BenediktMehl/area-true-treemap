...
export interface SquarifyNode {
    ...
    rows: SquarifyRow[];
    hasLabel: boolean;
    /** Distance from the root (0 for the root node) — added so that a label-size function can use it. */
    depth: number;
    x0: number;
    y0: number;
    x1: number;
...
/** Resolves the floor-label strip size for a single folder node. */
export type LabelSizeResolver = (node: SquarifyNode) => number;

/** Floor-label size: either a fixed value or a per-node function. */
export type LabelLength = number | LabelSizeResolver;
...
function layoutNodeWithRows(
    ...
    // Adaptation: the label strip replaces the margin on the label side.
...
function squarifyNode(
    ...
    // Apply margin; the label strip replaces the margin on the label side.
    let x0 = parent.x0 + margin;
    ...
    if (scale) {
        if (nodes && nodes.length > 0) {
            ...
            for (const child of nodes) {
                if (child.value !== undefined && !Number.isNaN(child.value)) {
